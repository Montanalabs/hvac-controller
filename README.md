...
- **Closed type:** `type Decision = SetMode(Mode) | OffMode` where `Mode` is closed
- **Sink / capability:** `grant setMode` → `privileged { setMode(d) }`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/hvac-controller/demo.sh
```
...
- `hvac-controller_safe.eide` — the correct design.
- `hvac-controller_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
